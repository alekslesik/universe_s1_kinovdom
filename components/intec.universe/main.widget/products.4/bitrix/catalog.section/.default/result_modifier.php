    ...
    foreach ($arResult['ITEMS'] as &$arItem) {
        ...
        // Код свойства, значение которого вы хотите получить
        $propertyCode = 'CATEGORY'; // Замените на код вашего свойства

        // Получение свойства элемента
        $res = CIBlockElement::GetProperty($iblockId, $elementId, array("sort" => "asc"), array("CODE" => $propertyCode));

        // Получение первого значения свойства (если оно существует)
        if ($ob = $res->GetNext()) {
            $arCategory = $ob['VALUE_XML_ID'];
        } else {
